...
/**
 * 추출 모델 — **비용 최적화가 이 선택의 이유다** (2026-09-20, 사용자 결정).
 *
 * 이 작업은 화면에 보이는 글자를 JSON으로 옮기는 것이지 추론이 아니다. 그리고 올라오는
 * 이미지 장수가 계속 늘어날 예정이라, 장당 단가가 그대로 운영비가 된다.
 *
 * Opus 5 대비 입력 1/5·출력 1/5 단가인데, 실제 차이는 그보다 크다: Opus 5는 `thinking`을
 * 생략하면 adaptive thinking이 **기본으로 켜지고** 그 사고 토큰이 출력 단가($25/1M)로
 * 청구된다. Haiku 4.5는 `thinking`을 생략하면 사고 자체를 하지 않아 그 항목이 0이 된다.
 * 장당 대략 60원 → 5원 수준.
 *
 * 대가: 빽빽한 목록에서 작은 글씨를 Opus만큼 정확히 읽지는 못한다. 이 앱은 그 실수를
 * 사람이 바로 고칠 수 있는 구조라(카드의 "정보 고치기", "직접 만들기") 감당 가능한 교환이다.
 * 정확도가 아쉬우면 `claude-sonnet-5`로 올리면 된다 — 아래 output_config 주석도 함께 볼 것.
 */
const MODEL = "claude-haiku-4-5";
/**
 * 상한은 비용이 아니라 잘림 방지용이라 넉넉히 둔다 — 잘린 JSON은 파싱 실패가 되어 통째로 버려진다.
 *
 * 지금 모델(Haiku 4.5)은 사고를 하지 않으므로 이 예산은 전부 출력 몫이다 — 60칸 목록도 넉넉하다.
 *
 * **사고하는 모델로 올릴 때는 이 값을 출력 길이만으로 잡으면 안 된다.** Opus 5·Sonnet 5는
 * 사고 토큰이 max_tokens에 함께 잡힌다. 예전 값(8192)은 "상품 1건 = 200토큰"이라는 출력 기준으로만
 * 잡혀 있어서, 상품 30칸짜리 목록에서 사고 단계에 예산이 바닥나 텍스트 블록이 아예 안 나오거나
 * JSON이 중간에 잘릴 수 있는 상태였다. max_tokens는 상한일 뿐 쓴 만큼만 과금되므로 넉넉히 둔다.
 */
    ...
    const response = await anthropic.messages.create(
      {
        ...
        // `output_config.effort`도 `thinking`도 보내지 않는다. Haiku 4.5는 사고가 기본으로
        // 꺼져 있어 낮출 것이 없고, effort는 이 모델에서 거부될 수 있다 — 보내면 400이 나고
        // 그건 api-error로 떨어져 모든 캡처가 실패한다. 조절할 것이 없으니 아예 뺀다.
        // (Sonnet 5 이상으로 올릴 때는 `output_config: { effort: "low" }`를 다시 붙일 것.)
